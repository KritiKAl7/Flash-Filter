...
enum {
    ...
    M_PROCESS_JBILATERAL=32,
    M_PROCESS_WHITEBALANCE=33,
...
int make_menu ()
{
    ...
    glutAddMenuEntry( "Denoising", M_PROCESS_JBILATERAL);
    glutAddMenuEntry("White Balancing", M_PROCESS_WHITEBALANCE);
...
void process_func (int value)
{
    ...
    case M_PROCESS_JBILATERAL: // enum #32
        {
            char filename[MAX_NAME];
            // we don't do a lot of checks here; i.e. second image and
            // mask valid images and the same size as current image
            ...
            secondImage->read(filename);
            checkStream(cin);
            resultImage = sg_final(currentImage, secondImage);
            delete secondImage;
            break;
        }
            
        case M_PROCESS_WHITEBALANCE: // enum #33
        {
            char filename[MAX_NAME];
            // we don't do a lot of checks here; i.e. second image and
            // mask valid images and the same size as current image
            ...
            secondImage->read(filename);
            checkStream(cin);
            resultImage = sg_whiteBalance(currentImage, secondImage);
            delete secondImage;
            break;
        }
