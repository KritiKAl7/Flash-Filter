...
Image* ip_composite (Image* noflash, Image* flash,
                      Image* nothing)
{
    int width = noflash->getWidth();
    int height = noflash->getHeight();
    
    Image* aFinal = new Image(width, height);
    Image* mask = sg_mask(noflash, flash);
    Image* aNR = sg_jBilateral(noflash, flash);
    Image* fBase = sg_jBilateral(flash, flash);
    
    Image* aBase = sg_jBilateral(noflash, noflash);
...
Image* sg_final (Image* noflash, Image* flash)
{
    int width = noflash->getWidth();
    int height = noflash->getHeight();
    ...
    Image* aNR = sg_jBilateral(noflash, flash);
    Image* fBase = sg_jBilateral(flash, flash);
    Image* fDetail = sg_detail(flash, fBase);
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            double red = (1.0-mask->getPixel_(x, y, 0))*aNR->getPixel_(x, y, 0)*fDetail->getPixel_(x, y, 0) + (mask->getPixel_(x, y, 0)*aBase->getPixel_(x, y, 0));
            double green = (1.0-mask->getPixel_(x, y, 1))*aNR->getPixel_(x, y, 1)*fDetail->getPixel_(x, y, 1) + (mask->getPixel_(x, y, 1)*aBase->getPixel_(x, y, 1));
            double blue = (1.0-mask->getPixel_(x, y, 2))*aNR->getPixel_(x, y, 2)*fDetail->getPixel_(x, y, 2) + (mask->getPixel_(x, y, 2)*aBase->getPixel_(x, y, 2));
            
            aFinal->setPixel_(x, y, 0, red);
            aFinal->setPixel_(x, y, 1, green);
            aFinal->setPixel_(x, y, 2, blue);
...
Image* sg_detail (Image* src1, Image* src2){
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            double red = (src1->getPixel_(x, y, 0) + sigma)/(src2->getPixel_(x, y, 0) + sigma);
            double green = (src1->getPixel_(x, y, 1) + sigma)/(src2->getPixel_(x, y, 1) + sigma);
            double blue = (src1->getPixel_(x, y, 2) + sigma)/(src2->getPixel_(x, y, 2) + sigma);
            
            tempImage->setPixel_(x, y, 0, red);
            tempImage->setPixel_(x, y, 1, green);
            tempImage->setPixel_(x, y, 2, blue);
        }
    }
...
Image* sg_jBilateral (Image* src, Image* src2){
...
Image* sg_whiteBalance (Image* noflash, Image* flash)
{
    int width = noflash->getWidth();
    int height = noflash->getHeight();
    Image* tempImage = new Image(width, height);
    ...
    double cBlue = 0.0;
    double cGreen = 0.0;
    double pixelCountR = 0.0;
    double pixelCountG = 0.0;
    double pixelCountB = 0.0;
    
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            
            double redDiff = flash->getPixel_(x, y, 0) - noflash->getPixel_(x, y, 0);
            double greenDiff = flash->getPixel_(x, y, 1) - noflash->getPixel_(x, y, 1);
            double blueDiff = flash->getPixel_(x, y, 2) - noflash->getPixel_(x, y, 2);
            
            if (redDiff>=0.1 && noflash->getPixel_(x,y,0)>=0.1) {
                cRed += noflash->getPixel_(x,y,0) / redDiff;
                ++pixelCountR;
            }
            
            if (greenDiff>=0.1 && noflash->getPixel_(x,y,1)>=0.1) {
                cGreen += noflash->getPixel_(x,y,1) / greenDiff;
                ++pixelCountG;
            }
            
            if (blueDiff>=0.1 && noflash->getPixel_(x,y,2)>=0.1) {
                cBlue += noflash->getPixel_(x,y,2) / blueDiff;
                ++pixelCountB;
            }
        }
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            double red = noflash->getPixel_(x, y, 0) / cRed;
            double green = noflash->getPixel_(x, y, 1) / cGreen;
            double blue = noflash->getPixel_(x, y, 2) / cBlue;
